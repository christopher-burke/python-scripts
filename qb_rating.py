...
class Nfl_Qb_Rating(object):
	...
	def __init__(self, com=0, att=0, yds=0, tds=0, ints=0):
		""" Quaterback Object to calculate all the parts that go into 
			the NFL QB Rating.
	...
	def qb_rating(self):
		""" Calculates and prints the NFL QB rating """
		...
		rule_one_two 	= lambda x : 2.375 if x > 2.375 else 0 if x < 0 else x
		#Rule three has a max of 2.375
		rule_three		= lambda x : 2.375 if x > 2.375 else x
		#Rule four has a min of 0
		rule_four		= lambda x : 0 if x < 0 else x
