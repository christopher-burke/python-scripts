...
''' Return the delta of the current date baased on the past/futrue and number of days/weeks.  '''
...
def time_diff_print(date_in):
    ...
    print '{0} {1}'.format('%d days %s' % (time_diff(date_in)),date_in.strftime("%B %d, %Y"))
...
if __name__ == "__main__":
    ...
    if kwargs['n'] == 1:
        print "{n} %s in the {time}".format(**kwargs) % (kwargs['time_frame'][:-1])
    else:
